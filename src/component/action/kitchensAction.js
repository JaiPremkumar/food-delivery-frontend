...
export const getSingleKitchen = (id)=>async(dispatch)=>{
    try {
        ...
        const {data} = await axios.get(`https://backend-food-delivery-3aba.onrender.com/api/v1/kitchen/${id}`) 
...
export const createKitchen =(formData)=> async(dispatch)=>{
    try {
        ...
        const {data} = await axios.post(`https://backend-food-delivery-3aba.onrender.com/api/v1/kitchen`,formData,config) 
...
export const updateKitchen =(id,name,price,description,kitchenName,image,categories)=> async(dispatch)=>{
    try {
        dispatch(newkitchenRequest())
        const {data} = await axios.put(`https://backend-food-delivery-3aba.onrender.com/api/v1/kitchen/${id}`,{name,price,description,kitchenName,image,categories}) 
        dispatch(newkitchenSuccess(data))
        console.log(data)
    } catch (error) {
        dispatch(newkitchenFailed(error.response.data.message))
