...
export const createProduct = (formData)=>async(dispatch)=>{
    try {
        ...
        const {data} = await axios.post(`https://backend-food-delivery-1.onrender.com/api/v1/product/new/`,formData,config) 
        console.log(data)
...
export const updateProduct = (id,name,price,image,seller,categories,description)=>async(dispatch)=>{
    try {
        ...
        const {data} = await axios.put(`https://backend-food-delivery-1.onrender.com/api/v1/update/${id}`,{name,price,image,seller,categories,description}) 
        console.log(data)
        dispatch(productCrtSuccess(data))
...
export const isReview=(id,rating,comment)=> async(dispatch)=>{ 
    try {
        ...
        const {data} = await axios.put(`https://backend-food-delivery-1.onrender.com/api/v1/review`,{rating,comment,id}) 
        console.log(data)
