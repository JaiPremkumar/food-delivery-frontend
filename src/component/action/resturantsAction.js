...
export const getSingleResturant =(id)=> async(dispatch)=>{
    try {
        ...
        const {data} = await axios.get(`https://backend-food-delivery-1.onrender.com/api/v1/rest/${id}`) 
...
export const createResturant =(formData)=> async(dispatch)=>{
    try {
        ...
        const {data} = await axios.post(`https://backend-food-delivery-1.onrender.com/api/v1/rest/`,formData,config) 
